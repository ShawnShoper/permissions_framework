		...
		this.init = function(){
			$(that.obj).die().live('change', function(){
				try{
					...
					var $selected = $(that.obj).next().find('.daq-drop-ul > li.selected').eq(0);
					var selectedVal = $selected.find('a').eq(0).attr('tabindex');
					var selectedOption = $(that.obj).find('option').eq($selected.index());
					var selectedText = $selected.find('span.text').eq(0).text().trim();
